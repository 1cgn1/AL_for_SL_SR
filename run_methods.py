...
#防止进程递归创建
def run_experiment(max_n_jobs, # 最多同时使用的cpu核心数，在我们的服务器上绝对不能超过40，最好不要超过三十，和其他人共用服务器时适当减少；dnn需要单独跑，跑dnn时请设置此参数为1
model_list, # 使用什么模型跑代码，备选包括['lr','rf','xgb','dnn','df','svm']
acquisition_function_list,# 使用何种获取函数，备选项包括["random", "exploration", "exploitation", "dynamic", "dynamicbald", "bald", "similarity"]
cell_line_list,# 跑哪些细胞系，备选项包括['A549','Hela','Jurkat','K562']
scores_list,# 标签数据中的打分标准，备选包括['sensitive','strong']
feature_type_list,# 特征数据类型，备选包括['function','protein']
dir_name, # 结果将储存在这个文件夹下，理论上可自由填写
positive_labels_list,# 预测的正标签，备选包括[0,1]分别对应SL和SR
n_start_list,# 初始训练集大小，理论上可自由填写
batch_size_list,# 每轮捕获的样本数量，理论上可自由填写
random_seed_list,# 随机种子，理论上可自由填写
# pool_ratio = 1#为了节省算力，在大细胞系里我们会考虑降低这个值，但后面会有单独逻辑判定，一般情况下此参数保持注释即可
save_genes , # 如果为True，则会额外生成一个文件记录每一轮获取的样本名称，否则只记录获取的样本数量
cold_start,# 如果为True，则会以冷启动方式启动，即在初始训练集出现过的样本不会在样本池中出现
one_cycle,# 如果为Ture，则模型会一次性学习one_cycle_num轮主动学习同样数量的样本并进行预测，作为对照组
max_cycle,# 最大学习轮数
ensemble_size, # 模型集群里有多少个不同随机初始化的模型，一般不需要动
learning_rate , # 学习率，仅对dnn模型生效
dropout_rate, # 模型每轮drop掉的神经元比例，设置较大时可缓解过拟合症状
epochs, # dnn模型训练的轮数
